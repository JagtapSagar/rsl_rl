...
class MultiTeacherDistillation:
    ...
    Compatible with current TensorDict observation system and training infrastructure.
    """
    ...
    def update(self):
        ...
        for epoch in range(self.num_learning_epochs):
            if hasattr(self.policy, 'reset'):
                self.policy.reset()
            if hasattr(self.policy, 'detach_hidden_states'):
            ...
            for obs, actions, privileged_actions, dones in self.storage.generator():
                ...
                if self.diversity_loss_coef > 0:
                    diversity_loss = self.policy.get_teacher_diversity(obs)
    ...
    def get_teacher_individual_losses(self, obs):
        """Get individual losses for each teacher (for analysis/debugging)."""
        with torch.no_grad():
            student_actions = self.policy.act_inference(obs)
            individual_losses = []

            teacher_obs = self.policy.get_teacher_obs(obs)
            teacher_obs = self.policy.teacher_obs_normalizer(teacher_obs)
